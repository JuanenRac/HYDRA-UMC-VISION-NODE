...
def test_validate_frame_oversized_body_is_rejected_before_reading_it_all(tmp_path: Path) -> None:
    ...
    # large an honest frame should be, so a body far past that is rejected
    # without ever holding the whole oversized buffer in memory.
    width, height, channels = 4, 4, 3
    expected = width * height * channels
    data = bytes(expected * 10)  # far larger than this frame could ever honestly be
    with running_server(tmp_path) as base:
        status, body = _post(f"{base}/validate-frame?width={width}&height={height}&channels={channels}", data)
        assert status == 400
        assert str(expected + 1) in body["error"]
